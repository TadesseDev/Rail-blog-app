class Post < ApplicationRecord
  ...
  validates :likesCount, numericality: { only_integer: true }, comparison: { greater_than_or_equal_to: 0 }
  def self.update_post_counter(user)
    user.update(postCount: Post.where(user: user).count)
  end

  def self.most_recent_comments(post)
    Comment.includes(:user).where(comments: {post: post}).order(created_at: :DESC).limit(5)
  end
end
